...
class BatchNormWrapper(paddle.nn.Layer):
    def __init__(self, layer):
        ...
        initializer = paddle.nn.initializer.Assign(
            np.identity(self._channel).reshape(
                [self._channel, self._channel, 1, 1]))
        compactor_attr = paddle.ParamAttr(
            name=compactor_name,
            ...
            need_clip=False)
        self.compactor = paddle.nn.Conv2D(
            self._channel,
            self._channel,
            1,
            bias_attr=False,
...
class ResRepPruner():
    def __init__(self, model, inputs, dtypes, opt, target_flops):
        ...
        self.conv2partners = {
        }  # {"conv2d_0.weight": [("conv2d_1.weight", 1, (c_out, c_in, k, k), op)]}
        self.brothers = {}
        ...
        for _name, _layer in self.model.named_sublayers():
            if isinstance(_layer, (paddle.nn.layer.norm.BatchNorm2D,
...
#        self.warmup_steps = 0 # debugggggggggggggggggggggggggggggggggggggggggg
    ...
    def mask_compactor_grad(self):
        for _conv, _compactor in self.conv2compactor.items():
            ...
            lasso_grad = _compactor.weight * (paddle.sum((_compactor.weight**2),
                                                         axis=(1, 2, 3),
                                                         keepdim=True)**(-0.5))
            _grad = _compactor.weight.grad * paddle.to_tensor(
                _mask) + lasso_grad * self.lasso_strength
...
#            self.update_mask()
        ...
        self.opt.step()
        self.compactor_opt.step()
        #        if self.current_step % 50 == 0:
        #            paddle.save(self.model.state_dict(), f"./{self.current_step}.pdparams")
...
#            sys.exit(0)
    ...
    def clear_grad(self):
        self.opt.clear_grad()
        self.compactor_opt.clear_grad()

    def minimize(self, loss):
        self.opt.minimize(loss)
        self.compactor_opt.minimize(loss)

    def state_dict(self):
        ret = {}
        ret.update(self.opt.state_dict())
        ret.update(self.compactor_opt.state_dict())
        return ret

    def set_state_dict(self, state_dict):
        self.opt.set_state_dict(state_dict)
        self.compactor_opt.set_state_dict(state_dict)
