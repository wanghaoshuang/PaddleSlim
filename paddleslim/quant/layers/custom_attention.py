...
class QuantizedCustomAttentionLayer(ConvertibleQuantedLayer):
    ...
    def forward(self, query_states, key_states, value_states, **kwargs):
        ...
        if self.enable_fake_quant:
            self.collect_kv_quant_policy(query_states, key_states, value_states, **kwargs)
        ...
        tmp_k = tensor.transpose(x=key_states, perm=perm)
        tmp_v = tensor.transpose(x=value_states, perm=perm)
        ...
        key_states = tensor.transpose(x=tmp_k, perm=perm)
        value_states = tensor.transpose(x=tmp_v, perm=perm)
        return self.layer(query_states, key_states, value_states, **kwargs)
