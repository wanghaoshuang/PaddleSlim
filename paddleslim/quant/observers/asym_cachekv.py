...
class AsymCacheKVObserverLayer(ChannelWiseObserver):
    ...
    def _cal_abs_max(self, inputs):
        ...
        """
        reduce_axis = tuple(
            [i for i in range(len(inputs.shape)) if i not in self._quant_axis])

        max_values = paddle.max(inputs.cast("float32"), axis=reduce_axis, keepdim=True)
        min_values = paddle.min(inputs.cast("float32"), axis=reduce_axis, keepdim=True)
